...
public class ApiAddToCardController {
    ...
    public ResponseEntity<Order> up(@PathVariable Long id, HttpServletRequest request) throws SQLException {
        HttpSession session=request.getSession();
    ...
    public ResponseEntity<Order> down(@PathVariable Long id, HttpServletRequest request) throws SQLException {
        HttpSession session=request.getSession();
        ...
        for (OrderDetail orderDetail:orderDetailList){
            if (orderDetail.getProduct().getId()==id){
                if ((orderDetail.getQuantity()-1)==0) {
